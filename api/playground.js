// api/playground.js
// Be Cre8v AI Playground — CORS-safe proxy with kid-safe prompts (Chat Completions)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!cfgResp.ok) {
      ...
      return res
        .status(500)
        .json({ message: "Failed to load products.json", details: txt.slice(0, 400) });
    }
...
Rules: ${guards}. No PII. No adult/violent themes. Keep replies short and encouraging.`;
    ...
    const oai = await fetch(CHAT_URL, {
      ...
      body: JSON.stringify({
        model: "gpt-4o-mini", // if you see model_not_found, change to "gpt-4o-mini-2024-07-18" or "gpt-4o"
        messages: [
